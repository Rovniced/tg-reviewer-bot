...
if __name__ == "__main__":
    ...
    application.add_handler(CallbackQueryHandler(query_decision, pattern=f"^{ReviewChoice.QUERY}"))
    application.add_handler(CallbackQueryHandler(withdraw_decision, pattern=f"^{ReviewChoice.WITHDRAW}"))
    application.add_handler(CallbackQueryHandler(retract_approved_submission, pattern=f"^{ReviewChoice.APPROVED_RETRACT}"))
    application.add_handler(CallbackQueryHandler(reject_reason, pattern=f"^REASON"))
    ...
    application.add_handler(CommandHandler("unban", unban_user, filters=~filters.UpdateType.EDITED_MESSAGE & filters.Chat(
            chat_id=ReviewConfig.REVIEWER_GROUP)))
    application.add_handler(CommandHandler("listban", list_banned_users, filters=~filters.UpdateType.EDITED_MESSAGE & filters.Chat(
            chat_id=ReviewConfig.REVIEWER_GROUP)))
    application.add_handler(CommandHandler("stats", submitter_stats, filters=~filters.UpdateType.EDITED_MESSAGE, ))
    application.add_handler(CommandHandler("reviewer_stats", reviewer_stats))
